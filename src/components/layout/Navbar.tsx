...
import { Search, ShoppingCart, User, Menu, Heart, MapPin, Phone, ChevronDown } from 'lucide-react';
        ...
        <div className="container mx-auto px-4 flex justify-between items-center">
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium">Call: 1800-266-0777</span>
            </div>
            <div className="flex items-center space-x-2">
        ...
        <div className="flex items-center justify-between h-20">
              ...
              <AnimatePresence>
                {showCategoriesMenu && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute top-full left-0 mt-2 w-80 bg-white shadow-xl rounded-lg border border-gray-200 py-4 z-50"
                  >
                    <div className="grid grid-cols-1 gap-1">
                      {categories.map((category) => (
                        <Link
                          key={category.name}
                          href={category.href}
                          className="px-6 py-3 text-sm text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors duration-200 font-medium"
                        >
                          {category.name}
                        </Link>
                      ))}
              ...
              <AnimatePresence>
                {showAgeMenu && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    exit={{ opacity: 0, y: 10 }}
                    className="absolute top-full left-0 mt-2 w-48 bg-white shadow-xl rounded-lg border border-gray-200 py-4 z-50"
                  >
                    <div className="grid grid-cols-1 gap-1">
                      {ageGroups.map((age) => (
                        <Link
          ...
          <div className="hidden md:flex items-center flex-1 max-w-lg mx-8">
            <div className="relative w-full">
              <Input
                type="text"
                placeholder="Search for toys, games & more..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="pl-12 pr-4 py-3 rounded-full border-2 border-gray-300 focus:border-red-500 transition-colors text-sm"
              />
            ...
            <Sheet>
              ...
              <SheetContent side="right" className="w-80">
                <div className="flex flex-col space-y-6 mt-8">
                  ...
                  {/* Age Groups */}
                  <div>
                    <h3 className="font-bold text-gray-800 mb-3">Shop by Age</h3>
                    <div className="space-y-2">
                      {ageGroups.map((age) => (
                        <Link
                          key={age.name}
            ...
            >
              <div className="relative">
                <Input
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-12 pr-4 py-3 rounded-full border-2 border-gray-300 focus:border-red-500"
                />
                <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              </div>
            </motion.div>
